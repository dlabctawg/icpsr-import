...
#Pipe
##P1: Redumping ICPSR as .RData files
...
Now let's also subset our two sources for metadata.
```{r}
...
```
So those are the three different subsets of our data. Storing them separately is the most space efficient way, but not necessarily the most convenient. Let's merge them into one big table by matching the the two diffrent keys, speechID and speakerID.

```{r merge1}
smsdt<-merge(x=smdt,y=ssdt,by.x='speakerID',by.y='id') # merge by the same key, which is called two different things!
invisible(smsdt[,id:=NULL]) # delete the redundant key
tables()
```
```{r merge2}
