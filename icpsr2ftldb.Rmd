...
rm(dt) # let's free up some memory by deleting the big table
```
...
```{r save-tab-delim}
write.table(sdt,file = '2pipe/33501-0015-Data-1ksubsamp.txt',quote = F,sep = '|',row.names = F)
```

Let's test that we can read it in exactly as we wrote it.
```{r test-save}
tdt<-fread(input = '2pipe/33501-0015-Data-1ksubsamp.txt',colClasses = c(speechID='character'))
setkey(tdt,speechID)
if(identical(sdt,tdt)) "Yay they're the same, we did it!"
rm(tdt)
```
